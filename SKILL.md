...
From URL:
```python
task = client.tasks.create(index_id="<index-id>", video_url="https://example.com/video.mp4")
print(f"Task: {task.id} (status: {task.status})")

# Wait for indexing to complete
task = client.tasks.wait_for_done(task_id=task.id)
print(f"Ready. Video ID: {task.video_id}")
```
...
print(f"Status: {task.status}, Video ID: {task.video_id}")
```

Statuses: `Validating` → `Pending` → `Queued` → `Indexing` → `Ready` (or `Failed`).

List recent tasks:
```python
for task in client.tasks.list():
    print(f"{task.id}  {task.status}  video={task.video_id}")
```

## List videos in an index

```python
for video in client.indexes.videos.list("<index-id>"):
...
- **Indexing is async** — videos take time to process. Use `client.tasks.wait_for_done()` to block until ready.
